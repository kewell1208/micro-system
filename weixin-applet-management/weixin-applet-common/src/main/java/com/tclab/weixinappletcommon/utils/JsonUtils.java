...
public class JsonUtils {
    ...
    public static String objectToJson(Object data) {
        try {
            String string = JSON.writeValueAsString(data);
            return string;
        } catch (JsonProcessingException e) {
            e.printStackTrace();
        }
     ...
     * @return
     */
    public static <T> T jsonToPojoNew(String jsonData, Class<T> beanType) {
        try {
            T t = JSON.readValue(jsonData, beanType);
            return t;
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("json转换失败: "+jsonData+" , "+beanType.getName());
        }
    }
